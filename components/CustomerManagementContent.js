// components/CustomerManagementContent.js
...
const CustomerFormModal = ({ customer, onClose, onSave, isOpen }) => {
    ...
    useEffect(() => { setFormData(customer || { name: '', phone: '', points: 0 }); }, [customer, isOpen]);
    if (!isOpen) return null;
...
export default function CustomerManagementContent() {
    ...
    if (authLoading || customersLoading) { // customersLoading là biến loading từ useCollectionData
        console.log("RENDER: Hiển thị màn hình tải Customer Management. authLoading:", authLoading, "customersLoading:", customersLoading);
    ...
    if (!user) { // Sau khi authLoading và customersLoading đã false
        console.log("RENDER: Không có người dùng, trả về null.");
        return null;
    }

    if (customersError) { // Hiển thị lỗi nếu có lỗi tải dữ liệu
        console.log("RENDER: Hiển thị lỗi tải dữ liệu khách hàng:", customersError.message);

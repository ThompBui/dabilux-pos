// components/ProductManagementContent.js
...
export default function ProductManagementContent() { // Đã đổi tên hàm
    const [user, authLoading] = useAuthState(auth);
    const router = useRouter();

    const [products, setProducts] = useState([]);
    const [productsLoading, setProductsLoading] = useState(true); // Đổi tên từ 'loading'
    ...
    // Lắng nghe dữ liệu
    useEffect(() => {
        console.log("DATA: Bắt đầu lắng nghe Products data...");
        const q = query(collection(db, 'products'), orderBy('name', 'asc'));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setProducts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
            setProductsLoading(false); // Đổi tên từ setLoadingData
            console.log("DATA: Products tải:", snapshot.docs.length);
        }, (error) => {
            console.error("DATA: Lỗi khi lấy products:", error);
            setProductsLoading(false); // Mark as loaded even on error
        });
        return () => {
            unsubscribe();
            console.log("DATA: Unsubscribe Product Management đã chạy.");
        };
    }, []);
    ...
    if (authLoading || productsLoading) { // Sử dụng productsLoading
        console.log("RENDER: Hiển thị màn hình tải Product Management. authLoading:", authLoading, "productsLoading:", productsLoading);

...
export default function Dashboard() {
    ...
    const [activePreset, setActivePreset] = useState('today'); // NEW: State cho các nút lọc nhanh
    ...
    useEffect(() => {
        const unsubscribers = [
            onSnapshot(query(collection(db, 'bills'), orderBy('createdAt', 'desc')), (snapshot) => setBills(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))),
            onSnapshot(query(collection(db, 'products'), orderBy('name', 'asc')), (snapshot) => setProducts(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))),
            onSnapshot(query(collection(db, 'customers'), orderBy('name', 'asc')), (snapshot) => setCustomers(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))),
        ];
    ...
    const memoizedData = useMemo(() => {
        ...
        const filteredBills = bills.filter(b => { const billDate = b.createdAt?.toDate(); return billDate && billDate >= from && billDate <= toDateWithTime; });
        const currentStats = calculateStats(filteredBills);
        const dailySalesMap = {};
        filteredBills.forEach(bill => {
            const billDate = bill.createdAt?.toDate();
            if (billDate) {
                const dateKey = billDate.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' });
                dailySalesMap[dateKey] = (dailySalesMap[dateKey] || 0) + (bill.totalAfterDiscount || 0);
            }
        });
        const chartData = Object.keys(dailySalesMap).sort((a,b) => new Date(a.split('/').reverse().join('-')) - new Date(b.split('/').reverse().join('-'))).map(date => ({ date, DoanhThu: dailySalesMap[date] }));
        const productSales = {}; filteredBills.forEach(bill => { (bill.items || []).forEach(item => { productSales[item.id] = productSales[item.id] || { name: item.name, totalQuantity: 0, totalRevenue: 0 }; productSales[item.id].totalQuantity += item.quantity; productSales[item.id].totalRevenue += item.quantity * (item.price || 0); }); });
        const topSellingProducts = Object.values(productSales).sort((a, b) => b.totalQuantity - a.totalQuantity).slice(0, 5);
        const customerSpending = {}; filteredBills.forEach(bill => { if (bill.customer?.id) { customerSpending[bill.customer.id] = customerSpending[bill.customer.id] || { name: bill.customer.name, totalSpent: 0 }; customerSpending[bill.customer.id].totalSpent += bill.totalAfterDiscount || 0; } });
        const topCustomers = Object.entries(customerSpending).map(([id, data]) => { const customerInfo = customers.find(c => c.id === id); return { id, ...data, points: customerInfo?.points || 0 }; }).sort((a, b) => b.totalSpent - a.totalSpent).slice(0, 5);
    ...
    }, [bills, products, customers, activeRange, loading]);

    if (authLoading || loading || !memoizedData) {
    ...
    const handleFilter = () => {
        ...
        setActivePreset('custom'); // Đánh dấu là lọc tùy chỉnh

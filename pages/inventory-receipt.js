...
import Sidebar from '../components/Sidebar'; // Giả sử bạn có Sidebar component
...
export default function InventoryReceiptPage() {
    ...
    useEffect(() => {
        const unsubscribers = [
            ...
            onSnapshot(query(collection(db, 'stockReceipts'), orderBy('createdAt', 'desc')), (s) => setPastReceipts(s.docs.map(d => ({ id: d.id, ...d.data() }))))
        ];

...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const unsubscribes = [
            ...
            onSnapshot(query(collection(db, 'categories'), orderBy('name')), (snapshot) => setCategories(snapshot.docs.map(d => ({ id: d.id, ...d.data() })))),
            onSnapshot(query(collection(db, 'units'), orderBy('name')), (snapshot) => setUnits(snapshot.docs.map(d => ({ id: d.id, ...d.data() })))),
        ];
    ...
    if (authLoading || dataLoading) {
        return <div className="flex items-center justify-center h-screen bg-slate-100 dark:bg-slate-900"><p>Đang tải cài đặt...</p></div>;
    }
